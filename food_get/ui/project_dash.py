...
# Table data
table_data = [
    {'Data Source': 'Atlas Metrics', 'Collection Method': 'xlsx downloads', 'Data Year': '2010, 2015, and 2019'},
    {'Data Source': 'Census Data', 'Collection Method': 'API', 'Data Year': 'Census Tracts'},
    {'Data Source': 'csv/GeoJson download', 'Collection Method': '2010'},
    {'Data Source': 'Chicago Grocery Store Data', 'Collection Method': 'csv download', 'Data Year': '2018'},
...
def generate_layout(table_width):
    ...
    container_style = {'margin-top': '.25in', 'margin-bottom': '.25in', 'font-family': 'Helvetica, sans-serif'}
    header_style_second = {
    ...
    return html.Div([
        dcc.Location(id='url', refresh=False),  # Location component to track URL changes
        ...
        html.Div([
            html.H1(
                [
                    dcc.Link("About", href="/container1", style={'color': colors()['g1_color'], 'margin-right': '10px'}),
                    " | ",
                    dcc.Link("Historical Metric", href="/container2", style={'color': colors()['g1_color']}),
                    " | ",
                    dcc.Link("Recreated Metric", href="/container3", style={'color': colors()['g1_color']}),
                    " | ",
                    dcc.Link("Conclusion", href="/container4", style={'color': colors()['g1_color']}),
                    " | ",
                    html.A("GitHub", href=github_link, style={'color': colors()['g1_color'], 'margin-right': '10px'})
                ],
            ...
            html.Div([
                html.Div([
                    ...
                    dash_table.DataTable(
                        ...
                        data=table_data,
                        style_table={'height': '300px', 'overflowY': 'auto', 'padding-left': '0.25in', 'padding-right': '0.25in', 'width': '5.5in'},
                        style_cell={'textAlign': 'center'}
                    ),
                ], style={'width': '40%', 'float': 'left', 'background-color': colors()['g2_color'], 'margin-left': '0.25in'}),
                html.Div([
                    html.Iframe(srcDoc=open('folium_map.html', 'r').read(), width='100%', height='600px', style={'margin-left': '0.25in', 'margin-right': '0.25in'})
                ], style={'width': '50%', 'float': 'left'}),
            ], className='main-container'),
        ], style=container_style, id='container1'),

        # Container 2
        html.Div([
            html.Div([
                html.Div([
                    html.H3("Atlas Score", style={'margin-bottom': '0.5em', 'margin-left': '0.25in'}),
                    html.P(
        ...
        ], style=container_style, id='container2', hidden=True),

        # Container 3
        html.Div([
            html.Div([
                html.Div([
                    html.H3("Recreated Metric", style={'margin-bottom': '0.5em', 'margin-left': '0.25in'}),
                    html.P(
        ...
        ], style=container_style, id='container3', hidden=True),

        # Container 4
        html.Div([
            html.Div([
                html.Div([
                    html.H3("Conclusion", style={'margin-bottom': '0.5em', 'margin-left': '0.25in'}),
                    html.P(
        ...
        ], style=container_style, id='container4', hidden=True),
    ])
...
@app.callback(
    [Output(container_id, 'hidden') for container_id in ['container1', 'container2', 'container3', 'container4']],
    [Input('url', 'pathname')]
)
def update_container_visibility(pathname):
    if pathname is None or pathname == '/':
        return False, True, True, True  # Default to container1
    return pathname != '/container1', pathname != '/container2', pathname != '/container3', pathname != '/container4'
